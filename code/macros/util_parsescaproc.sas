	...
	*/
	...
	PROC SQL NOPRINT;
		...
		SELECT DISTINCT COALESCE(a.step,b.step,c.step) AS step
			...
			,d.elapsed
		FROM work.scadata2(where=(procname ^= '' %if(&l_impExpStepsCnt NE ) %then %do; AND NOT (step in (&l_impExpStepsExclude)) %end;)) as a
		FULL JOIN
			work.scadata2(where=(in ^= '' %if(&l_impExpStepsCnt NE ) %then %do; AND NOT (step in (&l_impExpStepsExclude)) %end;)) as b ON a.step=b.step
		FULL JOIN  
			work.scadata2(where=(out ^= '' %if(&l_impExpStepsCnt NE ) %then %do; AND NOT (step in (&l_impExpStepsExclude)) %end;)) as c ON a.step=c.step 
		LEFT JOIN
			work.scadata2(where=(elapsed>0 %if(&l_impExpStepsCnt NE ) %then %do; AND NOT (step in (&l_impExpStepsExclude)) %end;)) as d ON a.step=d.step 
		ORDER BY CALCULATED step
	  ;
	QUIT;

	%if(&l_impExpStepsCnt NE ) %then 
	%do;
	...
	%do;
		...
		DATA attrs_unique(drop=find_rc eq_rc eq in_out);
			...
			do until (eof);
				...
				/* %putHash (h,word5#word6#word7#lib_dsname#linenum#step#); */
				find_rc = HoH.find();
				/* put find_rc=; */
